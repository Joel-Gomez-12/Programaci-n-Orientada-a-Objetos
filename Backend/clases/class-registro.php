<?php
    class RegistrarUsuario{
        ...
        public static function verificarRegistro($correo, $contrasena){
                $contenidoArchivoRegistros = file_get_contents('../data/registros.json');
                $registros = json_decode($contenidoArchivoRegistros, true);
                for ($i=0; $i <sizeof($registros) ; $i++) { 
                        if ($registros[$i]["correo"]==$correo && $registros[$i]["contrasena"]==sha1($contrasena)) {
                                return $registros[$i];
                        }
                }
                return null;
